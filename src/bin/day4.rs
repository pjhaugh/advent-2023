...
fn parse_game(input: &'static str) -> IResult<&str, Game> {
    let (input, id) = map_res(preceded(
        preceded(tag("Card "), space0),
        digit1), str::parse::<u32>)(input)?;
    let (input, winning_nums) = preceded(ws(tag(":")),
                                         separated_list1(space1, map_res(digit1, str::parse::<u32>)))(input)?;

    let (input, my_nums) = preceded(ws(tag("|")),
                                         separated_list1(space1, map_res(digit1, str::parse::<u32>)))(input)?;
    Ok( (input, Game {
        id,
        winning_nums,
...
fn part_one(input: &'static str) -> Result<u32> {
    let mut res: u32 = 0;
    let games: Vec<Game> = input.lines().map(parse_game).map(|r| r.unwrap().1).collect();
    for game in games {
        let wins = game.my_nums.iter().filter(|n| game.winning_nums.contains(n)).count();
        if wins > 0 {
